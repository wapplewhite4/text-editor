...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for (String t: tokens) {
			...
			count++;
			//Increment numWords if token is a word
			if (isWord(t)){
				numWords++;
				numSyllables += countSyllables(t);
			} 
			//Increment numSentences if token is not a word OR
			//if the end of the list is reached without finding a sentence-ending punctuation
			if ((!isWord(t)) || count == tokens.size()) {
				numSentences++;
			}
